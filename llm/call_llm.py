...
def build_structured_prompt(full_code, node_id, code_line, in_set, gen_set, kill_set, out_set):
    return f"""
You are an expert in static program analysis and Reaching Definition Analysis (RDA). Use the following rules strictly:

- OUT = GEN ∪ (IN - KILL)
- GEN includes only definitions generated by the statement.
- KILL includes all definitions of variables that are redefined at this node.
- IN includes only definitions that reach the beginning of the node.

Given this code:

{full_code}

Evaluate this RDA entry strictly by the above rules:
